...
# =============================================================================
# Guardrails: Prevent Execution State Storage
# =============================================================================
...
def _assert_safe_value(value: str) -> None:
    """
    Assert that a value does not contain execution state keywords.
    
    Raises:
        ValueError: If execution state keywords are detected
    """
    value_lower = value.lower()
    for key in FORBIDDEN_KEYS:
        if key in value_lower:
            raise ValueError(
                f"SessionMemory must not store execution state: '{key}' detected in value"
            )
...
class SessionMemory(BaseModel):
    ...
    def set_recent_context(self, reply: str) -> None:
        ...
        Raises:
            ...
            ValueError: If reply contains execution state keywords
        """
        self._assert_not_locked()
        if reply:
            _assert_safe_value(reply)
    ...
    def add_conversation_entry(self, question: str, answer: str) -> None:
        ...
        Raises:
            RuntimeError: If session is locked
            ValueError: If question or answer contains execution state keywords
        """
        self._assert_not_locked()
        if question:
            _assert_safe_value(question)
        if answer:
            _assert_safe_value(answer)
